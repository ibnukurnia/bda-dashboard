...
const SynchronizedCharts: React.FC<SynchronizedChartsProps> = ({ dataCharts, height, width, chartTitle }) => {
      ...
      <div className="flex flex-col gap-2">
        ...
        {(() => {
          if (resDataChart.every((series) => series.data.length === 0)) {
            // if (dataCharts.every((series) => series.data.length === 0)) {
            return <Typography color={'white'}>NO DATA AVAILABLE</Typography>
          } else {
            return (
              <Chart
                options={chartOptions}
                // series={dataCharts.map((item) => ({ name: item.title, data: item.data })) as ApexAxisChartSeries}
